...
export const useCreditStore = defineStore('creditStore', {
  state: () => ({
    ...
    currentTransactionId: null as string | null,
    currentTransactionType: null as 'cash-in' | 'cash-out' | null,
  }),
  getters: {
    getCashin: (state) => state.cashin,
    getCashout: (state) => state.cashout,
    getCurrentBalance: (state) => state.currentBalance,
    getCurrentTransactionId: (state) => state.currentTransactionId,
    getCurrentTransactionType: (state) => state.currentTransactionType,
  },
  actions: {
    ...
    getCashinTransacById(transaction_id: string) {
      return this.getCashin.find((item) => item.transaction_id === transaction_id);
    },
    getCashoutTransacById(transaction_id: string) {
      return this.cashout.find((item) => item.transaction_id === transaction_id);
    },
    setCurrentTransactionId(transaction_id: string | null) {
      this.currentTransactionId = transaction_id;
    },
    setCurrentTransactionType(transaction_type: 'cash-in' | 'cash-out' | null) {
      this.currentTransactionType = transaction_type;
    },
  },
});
